...
1. **复读机**：同一聊天流内连续 N 条相同文本后，bot 跟读一次，之后进入冷却期防止连环复读。纯数字、超短英文单词、命令消息不参与统计。
...
3. **发送后自评撤回**：bot 每条消息（含复读与主链路回复）发出后延迟数秒，由 LLM 自评是否明显不合适（易引战 / 严重答非所问 / 可能违规），判定"该撤"则自动通过 NapCat 撤回。每小时有撤回配额上限兜底防误撤。
